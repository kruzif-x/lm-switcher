                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    helpSection(number: "3", title: "Settings explained", id: "s3", proxy: proxy) {
                        ...
                        Group {
                            ...
                            helpEntry("omlx", "oMLX server binary — ONE process serves every model under the oMLX model dir on a single port. Leave empty to auto-detect: LM Switcher checks ~/AI/envs/omlx-env/bin/omlx first, then /opt/homebrew/bin/omlx, then PATH.",
                                      detail: "A model folder under the oMLX root (org/model nesting up to 2 levels) is listed as an oMLX model and excluded from the plain MLX scan, so it is never double-listed. An oMLX server that is already running is adopted instead of spawning a second one.")
                            helpEntry("oMLX model dir · oMLX port", "Root folder scanned for oMLX-served models (default ~/AI/models/omlx) and the shared port for the one oMLX server (default 8000 — clear of llama.cpp 8080, MTPLX 8085, DS4 8090).",
                                      detail: "oMLX is the one SHARED server: stopping any oMLX model stops the server for all of them (the omlx-server worker is killed by port listener, since it survives its parent). Sampling, MTP and vision settings for oMLX models are oMLX's own — they live in ~/.omlx/model_settings.json (mtp_enabled) and LM Switcher passes no sampling flags for oMLX.")
                            ...
                            helpEntry("MTPLX port · Depth · Profile", "Port for MTPLX servers (default 8085). Depth = MTP draft depth 1–3 (start at 3). Profile: turbo (fastest), sustained, or exact.",
                                      detail: "Measured on an M2 Max 64 GB with Qwen3.8-27B-MTPLX-Optimized-Speed: ≈12 t/s plain autoregressive vs ≈18 / 19 / 25 t/s at depth 1 / 2 / 3 — depth 3 is ~2×. Run `mtplx tune --model <model folder> --depths 1,2,3` to benchmark the depths for a specific model; it saves the best depth for that model automatically (artifacts under ~/outputs/cli/tune/).")
                            helpEntry("MTPLX models in your library", "An MTPLX model is a folder containing mtplx_runtime.json next to config.json + *.safetensors. Put packs in the Models directory as REAL folders: `mtplx pull <repo>` downloads to ~/.mtplx/models/<org>--<name>, which the app will NOT see (it does not follow symlinks) — move the folder in. Update a pack in place with `mtplx models --update <repo> --installed-path <folder>`.",
                                      detail: "Only folders with mtplx_runtime.json are treated as MTPLX — plain MLX folders are untouched, and MTPLX models always use their own tokenizer template rather than the Chat template field. Depth and profile can be set per model in the Per-Model tab.")
                            helpEntry("MTPLX chat UI (no Open WebUI needed)", "Every loaded MTPLX model serves its own web chat at http://127.0.0.1:<port>/ while it runs — plus /dashboard/ for live stats and /docs for the OpenAI-compatible API reference. Nothing to install.",
                                      detail: "Launched with --no-auth and bound to 127.0.0.1 only, like every other backend here — reaching it from a phone or another Mac means relaunching that model by hand with --host 0.0.0.0 plus an --api-key. Use `mtplx connect openwebui --port <port> --model-id <id>` only if you specifically want Open WebUI (Docker: point it at http://host.docker.internal:<port>/v1).")
