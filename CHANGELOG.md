...
- **`llama status` mis-named entries.** Three defects compounded: MTPLX runs as `python -m mtplx.server.openai …`, so the generic ` -m ` extraction matched *python's* module flag and printed the whole argv as the model name; `\S` is not valid ERE on macOS bash 3.2 (the `--model` capture silently missed); and `local a="" b` resets only `a` on bash 3.2, so one entry displayed the previous entry's name. All three fixed — status now shows `Youssofal-…-FP16 :8085`, `Qwen3-VL-4B-Instruct-Q6_K.gguf :8084`, etc.
...
- **`docs/chat.html` — a single-file browser chat client for every engine** (port presets for llama.cpp/oMLX/MTPLX/DS4/mlx-serve, model list from `/v1/models`, streaming with thinking dimming, `?port=` / `?q=` params). It distinguishes "nothing listening" from "server blocks cross-origin calls" and points at the engine's own UI when it has one (MTPLX answers 403 to `file://`). Installed to `~/AI/tools/lm-switcher-chat.html` and bundled into the app as `Resources/chat.html`.
- **Help → 8. Browser access (chat in a web page)** — the per-engine UI map (llama.cpp `:port/`, MTPLX `:8085/` + `/dashboard/` + `/docs`, mlx-serve `:11234/`), the shipped chat page, the CORS table and the port list, with a button that opens the page in the default browser. Step 4 cross-references it; Command line → 9, Troubleshooting → 10.
- **README**: an engine-selection map rendered as an SVG image (generated from `docs/engine-map.html` by `scripts/export-engine-map-svg.py`) and a "Chat with your models in a browser" section with the per-engine UI/CORS table and worked examples (Qwen3.8-27B via MTPLX, DS4/oMLX via the chat page).
...
- **Help: "Which engine runs your model?"** — explains engine selection (format + owning tree) and opens a new HTML/SVG **engine-selection map** (`docs/engine-map.html`, deployed to `~/AI/tools/lm-switcher-engine-map.html` by install.sh) in an **in-app WKWebView window** (`EngineMapWindow`; the map ships in the app bundle as `Contents/Resources/engine-map.html` and install.sh also deploys a copy to `~/AI/tools`). A `file://` Link was tried first and shipped broken: it hands the file to LaunchServices, which cold-starts the default browser (Safari here) — slow, another Space, or nothing at all, indistinguishable from a dead click. Step 0/Step 4 help text now covers mlx-serve.
